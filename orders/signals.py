...
def order_notification_receiver(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        if instance.status == 'SHIPPED':
            courier = f" via {instance.courier_name}" if instance.courier_name else ""
            msg = f"Good news! Your order #{instance.order_number} has been shipped{courier}. Tracking number: {instance.tracking_number or 'Not Available'}."
            if instance.tracking_url:
                msg += f" Track your package here: {instance.tracking_url}"
            n_type = 'SHIPPING'
            ...
            if instance.delivery_proof_notes:
                msg += f" Delivery remarks: {instance.delivery_proof_notes}"
            n_type = 'ORDER'
        else:
